...
public class TajoPreparedStatement implements PreparedStatement {
   ...
   *
   */
  public TajoPreparedStatement(TajoClientInterface tajoClient,
                               String sql) {
  ...
  protected ResultSet executeImmediate(String sql) throws SQLException {
    ...
    try {
      ...
      resultSet = executeQueryAndGetResult(sql);
    } catch (Exception e) {
  ...
  private ResultSet executeQueryAndGetResult(final String sql) throws IOException {
    try {
      ...
      if (lastQueryId.equals(QueryIdFactory.NULL_QUERY_ID) && !response.hasHasResult()) {
        ...
        return tajoClient.getQueryResultAndWait(lastQueryId, TajoProxyClient.DEFAULT_FETCH_SIZE);
      }
    } catch (Exception e) {
