...
public class SimpleParser {
  ...
  public static List<org.apache.tajo.proxy.cli.ParsedResult> parseScript(String str) throws InvalidStatementException {
    org.apache.tajo.proxy.cli.SimpleParser parser = new org.apache.tajo.proxy.cli.SimpleParser();
    List<org.apache.tajo.proxy.cli.ParsedResult> parsedResults = new ArrayList<org.apache.tajo.proxy.cli.ParsedResult>();
    parsedResults.addAll(parser.parseLines(str));
    parsedResults.addAll(parser.EOF());
    return parsedResults;
  }

  public List<org.apache.tajo.proxy.cli.ParsedResult> parseLines(String str) throws InvalidStatementException {
    List<org.apache.tajo.proxy.cli.ParsedResult> statements = new ArrayList<org.apache.tajo.proxy.cli.ParsedResult>();
    int lineStartIdx;
    int idx = 0;
   ...
   * @throws org.apache.tajo.cli.InvalidStatementException
   */
  private List<org.apache.tajo.proxy.cli.ParsedResult> doProcessEndOfStatement(boolean endOfFile) throws InvalidStatementException {
    List<org.apache.tajo.proxy.cli.ParsedResult> parsedResults = new ArrayList<org.apache.tajo.proxy.cli.ParsedResult>();
    String errorMessage = "";
    if (endOfFile) {
    ...
    if (isTerminateState(state)) {
      ...
      if (state == ParsingState.META_EOS) {
        parsedResults.add(new org.apache.tajo.proxy.cli.ParsedResult(META, rawStatement, historyStatement));
        state = ParsingState.TOK_START;
      } else if (state == ParsingState.STATEMENT_EOS) {
        parsedResults.add(new org.apache.tajo.proxy.cli.ParsedResult(STATEMENT, rawStatement, historyStatement));
      } else {
